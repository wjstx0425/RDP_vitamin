...
class RingBuffer:
    ...
    def peek_last_n(self, n: int = 1) -> Tuple[List[Any], Optional[Exception]]:
        """
        Peek at the last n items in the buffer.
        Args:
            n: number of items to peek, default is 1
        Returns:
            (items, error): List of last n items (newest to oldest), or error if any
        """
        with self.mutex:
            # If buffer is empty
            if self.counter <= 1:
            ...
            available_items = min(self.counter - 1, self.size)
            n = min(n, available_items)

            # Calculate start position
            # head-1 is the last written position
            start_pos = (self.head - 1) if self.head > 0 else (self.size - 1)

            items = []
            for i in range(n):
                pos = (start_pos - i + self.size) % self.size
                if self.buf[pos] is not None:
                    items.append(self.buf[pos])
                else:
                    break
