...
class VideoRecorder(mp.Process):
    ...
    def start_wait(self, timeout: Optional[float] = None) -> None:
        if not self.ready_event.wait(timeout):
            raise TimeoutError(f"Video recorder startup timed out after {timeout}s")
    
    def end_wait(self, timeout: Optional[float] = None) -> None:
        self.join(timeout)
        if timeout is not None and self.is_alive():
            raise TimeoutError(f"Video recorder shutdown timed out after {timeout}s")
